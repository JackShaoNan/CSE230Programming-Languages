...
let rec build (rand,depth) = 
  ...
  | (_,0) -> if rand(0,1) = 0 then buildY() else buildX() (* base case *)
  | (_,d) -> let n = rand(0,7) in  (* recursive rule *)
    match n with
    | 0 -> buildY()
    | 1 -> buildX()
    ...
    | n -> buildCosine(build(rand, d-1));;


(* two more operation involved *)
let rec build2 (rand,depth) = 
  match (rand, depth) with
  | (_,0) ->  if rand(0,1) = 0 then buildY() else buildX()
  | (_,d) -> let n = rand(0,9) in
    ...
    | 5 -> buildTimes (build(rand,d-1), build(rand,d-1))
    | 6 -> buildThresh (build(rand,d-1), build(rand,d-1), build(rand,d-1), build(rand,d-1))
    | 7 -> buildSubtract (build(rand,d-1), build(rand,d-1))
    | 8 -> buildTan (build(rand,d-1))
    | n -> buildCosine(build(rand, d-1));;
