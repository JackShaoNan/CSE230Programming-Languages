...
% find the item list of Y, and check whether X is in list
available_at(X,Y) :- taqueria(Y,_,L), isin(X,L).

% use mahelper to find the item list, use bagof find all such list, 
% and then deduplicate,
mahelper(X) :- available_at(X,Y1), available_at(X,Y2), Y1\=Y2.
multi_available(X) :- bagof(X, mahelper(X), L), remove_duplicates(L, L1), isin(X,L1). 

% same as last question
worker_available_at(X,Y) :- taqueria(Y,B,C), isin(X,B).
owhelper(X) :- worker_available_at(X,Y1), worker_available_at(X,Y2), Y1\=Y2.
overworked(X) :- bagof(X, owhelper(X), L), remove_duplicates(L,L1), isin(X,L1). 

% find the ingredients list first and then recursively caculate the cost
cost_helper([],0).
cost_helper([H|T],K) :- cost(H,K1), cost_helper(T,K2), K is K1 + K2. 
total_cost(X,K) :- ingredients(X,L), cost_helper(L,K). 

% find the ingredients list first then check every elmt in L
hihelper(Lx,[]).
hihelper(Lx,[H|T]) :- isin(H,Lx), hihelper(Lx,T).
has_ingredients(X,L) :- ingredients(X,Lx), hihelper(Lx,L).

% same as the last question
aihelper(Lx,[]).
aihelper(Lx,[H|T]) :- \+ isin(H,Lx), aihelper(Lx,T).
avoids_ingredients(X,L) :- ingredients(X,Lx), aihelper(Lx,L). 


% just make use of last two functions
